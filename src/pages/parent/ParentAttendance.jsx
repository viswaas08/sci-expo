...
export default function ParentAttendance() {
  ...
  useEffect(() => {
    if (!childId) return;
    const q = query(collection(db, "attendance"), where("studentId", "==", childId));
    ...
    <div className="app-layout">
      ...
      <main className="main-content animate-fade-up">
        <div className="page-header">
          ...
          <p>Real-time attendance for {userData?.linkedStudentName || "your child"}</p>
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
          {[{ label: "Total Days", value: total, color: "var(--accent-blue)" }, { label: "Present", value: present, color: "var(--accent-green)" }, { label: "Absent", value: total - present, color: "var(--accent-red)" }, { label: "Percentage", value: `${pct}%`, color: pctColor }]
        ...
        <div className="glass-card" style={{ marginBottom: 24 }}>
          ...
          {pct < 75 && <p style={{ color: "var(--accent-orange)", fontSize: 13, marginTop: 10 }}>⚠ Attendance below 75%.</p>}
        </div>
              ...
              <tbody>
                {records.length === 0 ? (
                  ...
                  <tr key={r.id}>
                    ...
                    <td>{r.dept}</td><td>Year {r.year}</td><td>Sec {r.section}</td>
                  </tr>
                ))}
              </tbody>
            </table>
